...
url_error_message = "URL provided is Invalid. The URL should start" \
" with <span class='text-gray-400'>https://open.spotify.com/</span>"
...
@app.get("/songs/{filename}",
         response_class=Union[FileResponse, HTMLResponse])
...
async def processor(*, websocket: WebSocket, random:str):
    ...
    if valid_urls == []:
        ...
        return
    total_songs, url_data, directory_name  = await loop.run_in_executor(
        None,
        total_checker,
        valid_urls, random
    )
    ...
    kwargs = await loop.run_in_executor(
        None,
        downloader,
        url_data,
        random,
    ...
    await websocket.send_text(
        f"completed: /songs/[{random}] {directory_name}.zip"
    )
